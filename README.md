...
Note, the collection name must be an UUID which can be generated by ```uuid``` command in Linux/Mac. Use ```wmic path win32_computersystemproduct get UUID``` on Windows.
